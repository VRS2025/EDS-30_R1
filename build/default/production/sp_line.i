...
char detectLineState(char channel) {
    int adcValue = readADC(channel);
     ...
     if (adcValue >= 200 && adcValue <= 230)
        return 1;
    else if (adcValue >= 310 && adcValue <= 370)
        return 2;
    else if (adcValue >= 80 && adcValue <= 145)
        return 3;
    else if (adcValue >= 0 && adcValue <= 195)
        return 4;
...
void handleLineStates(unsigned char en) {
    if (en) {
        ...
        if (config.mode == 2) {
            ...
            } else if (sp_errors & (1 << 4)) {
                img_view(35, 5, 46, _sp2_line_short_HE, 0);
            } else if (sp_errors & (1 << 5)) {
