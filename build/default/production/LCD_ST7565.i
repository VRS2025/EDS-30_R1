...
void LCD_Command(uint8_t command) {
    ...
    for (uint8_t i = 0; i < 8; i++) {
        PORTDbits.RD2 = 0;
        PORTDbits.RD3 = (command & 0x80) ? 1 : 0;
        command <<= 1;
        PORTDbits.RD2 = 1;
    }
}
...
void display_digit(uint8_t x, uint8_t y, uint8_t digit, uint8_t inv) {
    if (y < 1 || y > 7 || x >= 128 || digit > 0x11) return;
    lcd_gotoxy(x, y - 1);
    for (uint8_t i = 0; i < 8; i++) {
