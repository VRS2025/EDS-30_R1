...
void SetControlRegister(unsigned char potAddress, unsigned char value)
{
    MCP4561_Write(potAddress, 0x04, value);
}
...
{
    I2CStart();
    I2CSend((unsigned char)(address << 1));
    I2CSend(reg);
    I2CSend(data);
    I2CStop();
}
...
void SetWiperValue(unsigned char potAddress, unsigned char wiperNumber, unsigned char value)
{
    ...
    unsigned char reg = (wiperNumber == 0) ? 0x00 : 0x01;
...
float ConvertVolumeToDB(unsigned char volume) {
    float normalizedValue = (255 - volume) / 255.0f;
    float dbValue = (float)(-70.0 + normalizedValue * (0.0 - -70.0));
