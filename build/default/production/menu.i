...
void menu1(void) {
    if (m1 == 1) {
        unsigned char y = 0;
        unsigned char tr[3] = {1, 0, 0};
        unsigned char m_neg[3] = {1, 0, 0};
...
Display_Picture(dry_menu_1_HE);
    ...
    while(m3==1) {
        ...
        if (_left2.click == 1 || m3 == 0) {
            ...
            EEPROM_Write(0x04 , counter[0].Zones);
            EEPROM_Write(0x06 , counter[0].MSG);

            EEPROM_Write(0x05 , counter[1].Zones);
            EEPROM_Write(0x07 , counter[1].MSG);
...
void _rs_comm_(void){
    ...
    unsigned char slave_count = config.slave_count;
    unsigned char in_master_mode = 0;
    unsigned char x = 0;
    ...
    while(rs_ != 1){
        ...
        if (_right1.click == 1) {
            ...
            if (x == 0) {
                config._485++;
                if (config._485 > 21) config._485 = 0;
            }
            if (x == 1 && in_master_mode) {
                slave_count++;
                if (slave_count > 21) slave_count = 1;
            }
        }
        ...
        if (_right2.click == 1) {
            ...
            if (x == 0) {
                config._485--;
                if (config._485 == 255 || config._485 == 0) config._485 = 21;
            }
            if (x == 1 && in_master_mode) {
                slave_count--;
                if (slave_count < 1) slave_count = 20;
            }
        }
        ...
        if (_left1.click == 1){
            ...
            if (backup_addr != config._485 || config.slave_count != slave_count) {
                EEPROM_Write(0x13, config._485);
                config.slave_count = slave_count;
                EEPROM_Write(0x19, slave_count);
            }
            rs_ = 1;
            lcd_clear();
            first_screen();
...
void _audio_sett(void) {
   ...
   if(config.mic1_mode == 0){
      ...
      shiftRegisterData &= ~((unsigned int) (1 << 12));
    ...
    while(aud != 1) {
        ...
        if (_right2.click == 1) {
            ...
            if (x == 3) {
                ...
                SetWiperValue(0x2f, 0, config.mic3_volume);
            }
            if (x == 4) {
 ...
 lcd_gotoxy(15, 7); sprintf(scr_buff, " MSG: %3d ", config.msg_volume); lcd_putstr(scr_buff, neg4);
        ...
        if (x == 0 ) {
            jq8900_stop();
            shiftRegisterData &= ~((unsigned int) (1 << 0));
            shiftRegisterData &= ~((unsigned int) (1 << 1));
            shiftRegisterData &= ~((unsigned int) (1 << 2));
            shiftRegisterData &= ~((unsigned int) (1 << 3));
            shiftRegisterData &= ~((unsigned int) (1 << 4));
            shiftRegisterData &= ~((unsigned int) (1 << 5));
            shiftRegisterData &= ~((unsigned int) (1 << 6));
            shiftRegisterData &= ~((unsigned int) (1 << 7));
            shiftRegisterData &= ~((unsigned int) (1 << 8));
            shiftRegisterData &= ~((unsigned int) (1 << 9));
            shiftRegisterData |= ((unsigned int) (1 << 10));
            shiftRegisterData &= ~((unsigned int) (1 << 11));
...
# 826 "menu.c"
        ...
        if (_left2.click == 1 || aud == 1) {
            ...
            if (_mgs_vol_backup != config.msg_volume) {
                EEPROM_Write(0x15, config.msg_volume);
            }
            if (_mic1_vol_backup != config.mic1_volume) {
                EEPROM_Write(0x16, config.mic1_volume);
            }
            if (_mic2_vol_backup != config.mic2_volume) {
                EEPROM_Write(0x17, config.mic2_volume);
            }
            if (_mic3_vol_backup != config.mic3_volume) {
                EEPROM_Write(0x18, config.mic3_volume);
            }
            lcd_clear();
            first_screen();
        }
...
void sys_timer_set(void) {
    ...
    unsigned char backup_mode = config.mode;
    ...
    while (m_t != 1) {
        ...
        if (_left2.click) {
            ...
            if (backup_line_timer != config.line_timer) {
                EEPROM_Write(0x14, config.line_timer);
...
void adc_monitor(void) {
    ...
    while (!debug_screen) {
        ...
        lcd_gotoxy(75, 0); sprintf(scr_buff, "AC:  %d", PORTBbits.RB0); lcd_putstr(scr_buff,0);
        lcd_gotoxy(75, 1); sprintf(scr_buff, "BATT:%d", PORTAbits.RA2); lcd_putstr(scr_buff,0);
        lcd_gotoxy(75, 3); sprintf(scr_buff, "F_AMP:%d", PORTBbits.RB2); lcd_putstr(scr_buff,0);
...
void adc_menu_MIC1(void) {
    ...
    unsigned char editMax = 0;


    unsigned char micChannels[3] = {17, 14, 3};
    int adcValues[3];
    char buf[10];
    PORTCbits.RC1 = 1;
    ...
    while (!_left2.click) {
        ...
        if(!editing){
            ...
            if(_left1.click){ _left1.click=0; editing=1; editMax=0; }
        } else {
